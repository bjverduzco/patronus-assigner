...
app.controller('PatronusAssignerController', function($http) {
  ...
  function handlePersonSuccess(response) {
    ...
    for (var i = 0; i < vm.people.length; i++) {
      if(vm.people[i].first_name != null && vm.people[i].last_name != null){
  ...
  function getPatronus() {
    $http.get('/patronus').then(handlePatronusSuccess, handleFailure);
  }
  ...
  vm.addPatronus = function() {
    ...
    console.log(data);
    $http.post('/patronus', data).then(function(response) {
      console.log(response);
      getPatronus();
    }, function(response) {
      console.log(response);
